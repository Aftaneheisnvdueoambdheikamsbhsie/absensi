        ...
        function updateAttendance() {
            ...
            lines.forEach(line => {
                const parts = line.trim().split(/\s+/); // Pisahkan dengan satu atau lebih spasi
                const className = parts.pop(); // Kelas selalu di bagian akhir
                const name = parts.join(' '); // Gabungkan kembali nama siswa yang terpecah
                if (name && className && parts.length > 1) {
                    newEntries.push([name, className]);
                    console.log("Nama:", name, "Kelas:", className); // Debugging untuk melihat hasil pemecahan
                }
            });
            ...
            newEntries.forEach(row => {
                ...
                if (!updated) {
                    let newRow = Array(attendanceData[0].length - 3).fill(''); // Kolom lain diisi kosong
                    newRow.unshift(row[0], row[1], 'P'); // Tambahkan nama, kelas, dan status kehadiran
